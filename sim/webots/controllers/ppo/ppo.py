...
class PPO_SV() extends Supervisor:
    
    def __init__():
        self.workers = [getFromDev("quad1"),
                        getFromDev("quad2"),
                        getFromDev("quad3"),
                        getFromDev("quad4"),]
        self.emitter = getFromDev("EMITTER")
    
    def step():
        
    
    def ppo(epochs=30,epoch_steps = 4000 , max_ep_len=500 ,pi_lr = 3e-4, vf_lr=1e-3,gamma=0.99,lam=0.97,pi_iters = 80,target_kl = 0.01,val_iters=80, clip_ratio=0.2):
        print("init robot..")
        env = robo_env.Robot_Environment()
        
        act_dim = len(env.motors)
        obs_dim = len(env.sensors)
